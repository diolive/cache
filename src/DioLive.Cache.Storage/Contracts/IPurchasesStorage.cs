...
namespace DioLive.Cache.Storage.Contracts
{
	public interface IPurchasesStorage
	{
		...
		Task<Guid> AddAsync(Guid budgetId, string name, int categoryId, DateTime date, decimal cost, string? shop, string? comments);
		Task UpdateAsync(Guid id, int categoryId, DateTime date, string name, decimal cost, string? shop, string? comments);
